...
class BoxComposite5CoMResult:
    """Computed CoM metadata for a 5-slice composite box."""
    total_mass: float
...
class BoxComposite5MomentArmResult:
    """Moment-arm metadata relative to a grasp position on the x-axis."""
    grasp_x: float
    moment_arm_x: float
...
def compute_box_composite5_true_com(
    ...
    """
    Compute the true CoM of a 5-slice composite box along the x-axis.

    Assumptions
    -----------
    - The full box is centered at the origin.
    - The box is divided uniformly into 5 slices along the x-axis.
    - Only the slice masses differ across conditions.
    """
...
def compute_box_composite5_normalized_moment_arm(
    ...
    """
    Compute the signed and normalized moment arm relative to a grasp position.

    normalized_moment_arm_x = |true_com_x - grasp_x| / Lx
    """
    if len(full_size_xyz) != 3:
        raise ValueError("full_size_xyz must have exactly 3 elements.")
